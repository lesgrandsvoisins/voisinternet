...
.PHONY: help venv install setup migrate makemigrations fixtures superuser \
        ...
        pole-qmd-export pole-qmd-import project-qmd-export project-qmd-import
...
pole-qmd-export: ## Exporte un pôle en .qmd : make pole-qmd-export SLUG=civisme [LANG=fr] [OUT=chemin.qmd]
	$(LOADENV) $(PYTHON) manage.py export_pole_qmd $(SLUG) --lang=$(or $(LANG),fr) $(if $(OUT),--out=$(OUT))

pole-qmd-import: ## Importe/met à jour un pôle depuis un .qmd : make pole-qmd-import FILE=civisme.qmd
	$(LOADENV) $(PYTHON) manage.py import_pole_qmd $(FILE)

project-qmd-export: ## Exporte un projet en .qmd : make project-qmd-export SLUG=mon-projet [LANG=fr] [OUT=chemin.qmd]
	$(LOADENV) $(PYTHON) manage.py export_project_qmd $(SLUG) --lang=$(or $(LANG),fr) $(if $(OUT),--out=$(OUT))

project-qmd-import: ## Importe/met à jour un projet depuis un .qmd : make project-qmd-import FILE=mon-projet.qmd
	$(LOADENV) $(PYTHON) manage.py import_project_qmd $(FILE)
